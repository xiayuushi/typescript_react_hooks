...
const todosReducer = (state = initState, action: XxxTodosActionType): XxxTodosStateType => {
  if (action.type === 'TODOS/ADD_TODOS') {
    return [
      {
        id: Date.now(),
        name: action.name,
  ...
  if (action.type === 'TODOS/DEL_TODOS') {
    return state.filter(item => item.id !== action.id)
  }
  if (action.type === 'TODOS/CHANGE_TODOS') {
  ...
  if (action.type === 'TODOS/DELAY_DEL_TODOS') {
    return state.filter(item => item.id !== action.id)
  }
  return state
}
